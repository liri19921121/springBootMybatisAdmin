...
package com.admin.service.version.impl;

import com.admin.dao.version.VersionDao;
...
public class VersionServiceImpl implements VersionService {
	...
	@Transactional
	 public   int insert(ParameterMap pm){
		 return versionMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return versionMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(Long versionId){
		 return versionMapper.selectByPrimaryKey(versionId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  versionMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  versionMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.versionMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.versionMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteVersion(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.versionMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.versionMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.versionMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.versionMapper.getByConditionCount(pm);
	}
	...
	public Object add(ParameterMap pm,HttpSession session) throws Exception {
		try {
			...
			if (pm.get("isNew").toString().equals("1")){
				updateIsNew(pm);
			}
	...
	public Object edit(ParameterMap pm, HttpSession session) throws Exception {
		try {
			...
			Object isNew = pm.get("isNew");
			if (isNew != null || isNew.toString().equals("1")){
				//其它选项置0
                this.updateIsNew(pm);
			}
	...
	@Transactional
	public  int updateIsNew(ParameterMap pm){
		return this.versionMapper.updateIsNew(pm);
	}
