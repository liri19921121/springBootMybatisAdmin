...
//	private RedisTemplate<String, Object> redis;
	...
	public HashMap<String, Object> login(ParameterMap pm, HttpSession session) {
		...
		try {
			...
			pm.put("password", SHA.encryptSHA(psw));
			User user = userDao.getUserInfo(pm);
			if(user == null){
				return ResponseModel.getModel(ResultEnum.ERROR_INCORRECT_NAMEORPASSWORD, null);
			}
			if("lock".equalsIgnoreCase(user.getStatus())){
				return ResponseModel.getModel(ResultEnum.LOCK_ACCOUNT, null);
			}
			//获取用户权限
			...
			checkMenuRole(oneMenuList, uRole.getRights(),"admin".equals(user.getUsername()));
	...
	public HashMap<String, Object> add(ParameterMap pm,HttpSession session) {
		try {
			...
			pm.put("create_by", ((User) session.getAttribute(Const.SESSION_USER)).getUserId());
			userDao.saveUser(pm);
		} catch (Exception e) {
	 ...
	 *            用户权限值
	 */
	public void checkMenuRole(List<Menu> oneMenuList, BigInteger rights,boolean isAdmin) {
		for (Menu m : oneMenuList) {
			m.setHasMenu(RightsHelper.testRights(rights, m.getMENU_ID()) || isAdmin);
			if (m.isHasMenu()) {
				List<Menu> subList = m.getSubMenu();
				for (Menu subM : subList) {
					subM.setHasMenu(RightsHelper.testRights(rights, subM.getMENU_ID()) || isAdmin);
				}
			}
		}
	}
