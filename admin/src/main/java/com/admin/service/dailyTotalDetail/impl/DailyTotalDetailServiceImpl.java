...
package com.admin.service.dailyTotalDetail.impl;
...
import com.admin.dao.dailyTotalDetail.DailyTotalDetailDao;
...
@Service
@Transactional
public class DailyTotalDetailServiceImpl implements DailyTotalDetailService {
	
	/**
	 * this loger  can be used by service in anywhere . wzz
	 */
	@Autowired
	private DailyTotalDetailDao dailyTotalDetailMapper;
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	
	@Transactional
	 public  int deleteByPrimaryKey(java.lang.Long dailyTotalDetailId){
		 return  dailyTotalDetailMapper.deleteByPrimaryKey(dailyTotalDetailId);
	 }
	
	@Transactional
	 public   int insert(ParameterMap pm){
		 return dailyTotalDetailMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return dailyTotalDetailMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(java.lang.Long dailyTotalDetailId){
		 return dailyTotalDetailMapper.selectByPrimaryKey(dailyTotalDetailId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  dailyTotalDetailMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  dailyTotalDetailMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.dailyTotalDetailMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.dailyTotalDetailMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteDailyTotalDetail(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.dailyTotalDetailMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.dailyTotalDetailMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.dailyTotalDetailMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.dailyTotalDetailMapper.getByConditionCount(pm);
	}
	...
	public Object del(ParameterMap pm) throws Exception {
		...
		try {
			...
			isdel = this.deleteDailyTotalDetail(ids);
