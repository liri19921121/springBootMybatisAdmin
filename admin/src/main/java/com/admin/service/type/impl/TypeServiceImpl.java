...
package com.admin.service.type.impl;

import com.admin.dao.type.TypeDao;
...
import com.admin.service.type.TypeService;
...
public class TypeServiceImpl implements TypeService {
	...
	@Transactional
	 public   int insert(ParameterMap pm){
		 return typeMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return typeMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(Long typeId){
		 return typeMapper.selectByPrimaryKey(typeId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  typeMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  typeMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.typeMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.typeMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteType(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.typeMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.typeMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.typeMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.typeMapper.getByConditionCount(pm);
	}
