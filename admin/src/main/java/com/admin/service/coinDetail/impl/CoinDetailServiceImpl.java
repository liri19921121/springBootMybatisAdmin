...
package com.admin.service.coinDetail.impl;

import com.admin.dao.coinDetail.CoinDetailDao;
...
public class CoinDetailServiceImpl implements CoinDetailService {
	 ...
	 */
	@Autowired
	private CoinDetailDao coinDetailMapper;
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	
	@Transactional
	 public  int deleteByPrimaryKey(Long coinDetailId){
		 return  coinDetailMapper.deleteByPrimaryKey(coinDetailId);
	 }
	
	@Transactional
	 public   int insert(ParameterMap pm){
		 return coinDetailMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return coinDetailMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(Long coinDetailId){
		 return coinDetailMapper.selectByPrimaryKey(coinDetailId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  coinDetailMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  coinDetailMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.coinDetailMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.coinDetailMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteCoinDetail(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.coinDetailMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.coinDetailMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.coinDetailMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.coinDetailMapper.getByConditionCount(pm);
	}
	...
	public Object del(ParameterMap pm) throws Exception {
		...
		try {
			...
			isdel = this.deleteCoinDetail(ids);
