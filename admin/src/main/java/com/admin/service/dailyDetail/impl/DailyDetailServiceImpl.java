...
package com.admin.service.dailyDetail.impl;

import com.admin.dao.dailyDetail.DailyDetailDao;
...
import com.admin.service.dailyDetail.DailyDetailService;
...
public class DailyDetailServiceImpl implements DailyDetailService {
	 ...
	 */
	@Autowired
	private DailyDetailDao dailyDetailMapper;
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	
	@Transactional
	 public  int deleteByPrimaryKey(java.lang.Long dailyDetailId){
		 return  dailyDetailMapper.deleteByPrimaryKey(dailyDetailId);
	 }
	
	@Transactional
	 public   int insert(ParameterMap pm){
		 return dailyDetailMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return dailyDetailMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(java.lang.Long dailyDetailId){
		 return dailyDetailMapper.selectByPrimaryKey(dailyDetailId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  dailyDetailMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  dailyDetailMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.dailyDetailMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.dailyDetailMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteDailyDetail(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.dailyDetailMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.dailyDetailMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.dailyDetailMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.dailyDetailMapper.getByConditionCount(pm);
	}
	...
	public Object del(ParameterMap pm) throws Exception {
		...
		try {
			...
			isdel = this.deleteDailyDetail(ids);
