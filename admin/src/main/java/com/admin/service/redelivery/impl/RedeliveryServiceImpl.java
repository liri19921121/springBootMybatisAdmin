...
package com.admin.service.redelivery.impl;

import com.admin.dao.redelivery.RedeliveryDao;
...
@Service
@Transactional
public class RedeliveryServiceImpl implements RedeliveryService {
	
	/**
	 * this loger  can be used by service in anywhere . wzz
	 */
	@Autowired
	private RedeliveryDao redeliveryMapper;
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	
	@Transactional
	 public  int deleteByPrimaryKey(java.lang.Long redeliveryId){
		 return  redeliveryMapper.deleteByPrimaryKey(redeliveryId);
	 }
	
	@Transactional
	 public   int insert(ParameterMap pm){
		 return redeliveryMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return redeliveryMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(java.lang.Long redeliveryId){
		 return redeliveryMapper.selectByPrimaryKey(redeliveryId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  redeliveryMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  redeliveryMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.redeliveryMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.redeliveryMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteRedelivery(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.redeliveryMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.redeliveryMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.redeliveryMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.redeliveryMapper.getByConditionCount(pm);
	}
	...
	public Object del(ParameterMap pm) throws Exception {
		...
		try {
			...
			isdel = this.deleteRedelivery(ids);
