...
package com.admin.service.coinInfoName.impl;

import com.admin.dao.coinInfoName.CoinInfoNameDao;
...
@Service
@Transactional
public class CoinInfoNameServiceImpl implements CoinInfoNameService {
	
	/**
	 * this loger  can be used by service in anywhere . wzz
	 */
	@Autowired
	private CoinInfoNameDao coinInfoNameMapper;
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	
	@Transactional
	 public  int deleteByPrimaryKey(Long coinInfoNameId){
		 return  coinInfoNameMapper.deleteByPrimaryKey(coinInfoNameId);
	 }
	
	@Transactional
	 public   int insert(ParameterMap pm){
		 return coinInfoNameMapper.insert(pm);
	 }
	
	@Transactional
	 public  int insertSelective(ParameterMap pm){
		 return coinInfoNameMapper.insertSelective(pm);
	 }

	 public   ParameterMap selectByPrimaryKey(Long coinInfoNameId){
		 return coinInfoNameMapper.selectByPrimaryKey(coinInfoNameId);
	 }
	 
	@Transactional
	 public  int updateByPrimaryKeySelective(ParameterMap pm){
		 return  coinInfoNameMapper.updateByPrimaryKeySelective(pm);
	 }

	@Transactional
	 public  int updateByPrimaryKey(ParameterMap pm){
		 return  coinInfoNameMapper.updateByPrimaryKey(pm);
	 }
	...
	public boolean saveOrUpdate(ParameterMap pm,String operateType) throws Exception{
		...
		if("add".equals(operateType)){
			flag = this.coinInfoNameMapper.insert(pm) > 0 ? true : false;
		}
		
		if("edit".equals(operateType)){
			flag = this.coinInfoNameMapper.updateByPrimaryKey(pm) > 0 ? true : false;
		}
	...
	@Transactional
	public boolean deleteCoinInfoName(String ids) throws Exception {
		boolean flag = false;
		if(ids != null && !"".equals(ids.trim())){
			String[] id = ids.split(",");
			flag = this.coinInfoNameMapper.deleteByIds(id)> 0 ? true : false;
		}
		return flag;
	}
	
	public ParameterMap getByCondition(ParameterMap pm) throws Exception {
		return this.coinInfoNameMapper.getByCondition(pm);
	}

	
	public List<ParameterMap> getByConditionList(ParameterMap argmap) throws Exception {
		return this.coinInfoNameMapper.getByConditionList(argmap);
	}
	
	public long getByConditionCount(ParameterMap pm) throws Exception {
		return this.coinInfoNameMapper.getByConditionCount(pm);
	}
	...
	public Object del(ParameterMap pm) throws Exception {
		...
		try {
			...
			isdel = this.deleteCoinInfoName(ids);
