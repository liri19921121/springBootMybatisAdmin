...
package com.admin.controller.coinLang;
...
@Controller
@RequestMapping("/coinLang")
public class CoinLangController extends BaseController{
	
	private final static String qxurl="coinLang/list";
	...
	public Object list(@PathVariable(value="page",required=false)Integer pageNo,Model model){
		try {
			...
			Page<ParameterMap> page = PageHelper.startPage(pageNo, Const.PAGE_SIZE).doSelectPage(() -> {
				try {
					coinLangService.getByConditionList(pm);
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
			model.addAttribute("coinLang", page.getResult());
	...
	public Object getDetail() throws Exception{
		return coinLangService.getDetail(this.getParameterMap());
	}
	...
	public Object edit() throws Exception{
		return coinLangService.edit(this.getParameterMap(),this.getSession());
	}
	...
	public Object add() throws Exception{
		return coinLangService.add(this.getParameterMap(),this.getSession());
	}
	...
	public Object del() throws Exception{
		return coinLangService.del(this.getParameterMap());
	}
}